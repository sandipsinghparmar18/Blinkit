...
function EditSubCategory({ close, data: propData, fetchData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await Axios.put(
        `/api/subcategory/update/${data._id}`,
        data,
        {
          headers: {
      ...
      fetchData();
      addToast("Subcategory Edited successfully", "success");
      close();
    } catch (error) {
      console.error(error);
      addToast("Failed to Edit subcategory", "error");
    } finally {
      setLoading(false);
    }
  };
    ...
    <section className="fixed top-0 bottom-0 left-0 right-0 bg-neutral-800/80 flex items-center justify-center p-4">
      <div className="w-full max-w-5xl bg-white p-4 rounded">
        <div className="flex items-center justify-between gap-3">
          <h1 className="font-semibold">Edit Sub Categry</h1>
